...
export function StorageUsage({ promptsCount }: StorageUsageProps) {
  ...
  useEffect(() => {
    ...
    const calculateUsage = async () => {
      if (navigator.storage && navigator.storage.estimate) {
        const { usage: totalUsage, quota: totalQuota } =
  ...
  return (
    <div className="storage-usage-container" style={{ padding: '1rem' }}>
      <div
        className="storage-usage-header"
        style={{
          display: 'flex',
          alignItems: 'center',
          gap: '0.5rem',
      ...
      <p className="sidebar-section-note">
        Watch browser quota and sync status.
      </p>

      <div
        className="storage-usage-sub"
        style={{
          display: 'flex',
