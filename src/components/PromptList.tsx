...
function PromptCardBody({
  ...
  return (
    <div className="prompt-body-preview">
      {isExpanded ? (
        <>
          <pre className="expanded-body">{prompt.body}</pre>
...
export function PromptList({
  ...
  const historyPrompt = historyPromptId
    ? prompts.find((p) => p.id === historyPromptId)
    : null;
  ...
  const PromptRow = ({
    ...
    style,
    data,
  }: ListChildData<Prompt[]>): ReactElement | null => {
    const prompt = data[index];
    if (!prompt) return null;

    const isExpanded = expandedIds.has(prompt.id);
    const promptWorkspace = prompt.workspaceId
      ? workspaceMap[prompt.workspaceId]
      : null;
