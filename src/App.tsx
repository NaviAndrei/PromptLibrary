...
function App() {
  ...
  const [promptHistory, setPromptHistory] = useIndexedDB<PromptVersion>(
    'history',
    [],
    'prompt-history',
  );
  ...
  const handleSavePrompt = (
    ...
    if (editingPrompt) {
      ...
      const updatedPrompts = prompts.map((p) =>
        p.id === editingPrompt.id ? { ...p, ...promptData, updatedAt: now } : p,
      );
  ...
  const handleMovePromptToWorkspace = (
    promptId: string,
  ...
  const handleAddPromptFromTemplate = (template: PromptTemplate) => {
    const now = new Date().toISOString();
    const newPrompt: Prompt = {
      id: crypto.randomUUID(),
      title: template.name,
    ...
    setPrompts([newPrompt, ...prompts]);
    toast.success(`Prompt created from template "${template.name}"!`);
  };
      ...
      <div className="app-layout">
        <aside className="sidebar">
          <div className="sidebar-scroll">
            ...
            />
            <div className="sidebar-divider" />
            <TemplateManager onAddAsPrompt={handleAddPromptFromTemplate} />
            <div className="sidebar-divider" />
                ...
                >
                  ...
                  {allModels.some((m) => !LLM_MODELS.includes(m)) && (
                    <optgroup label="Other">
                      {allModels
                        .filter((m) => !LLM_MODELS.includes(m))
                        .map((m) => (
                          <option key={m} value={m}>
                            {m}
                          </option>
